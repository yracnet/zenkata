...
public class ResultReaderImpl implements ResultReader {
	...
	@Override
	public Result read(Object value) throws ResultException {
		...
		if (value instanceof String) {
			try {
				...
				LOGGER.log(Level.INFO, "XML Process: {0}", xml.replace("\n", "\\n"));
				StreamSource source = new StreamSource(new StringReader(xml));
				return (Result) UNMARSHALLER.unmarshal(source);
			} catch (JAXBException e) {
				throw new ResultException("Error Parse XML Result", e);
			}
		}
		return null;
	}
